...
def set_seed(seed=2024):
    ...
    # Disable TF32 on Ampere+ GPUs (RTX 30/40, A100, etc.) for full reproducibility
...
def mclust_R(adata, num_cluster, modelNames="EEE", used_obsm="emb_pca", random_seed=2020):
    ...
    if X.ndim != 2:
        raise ValueError(f"{used_obsm} must be 2D, got shape {X.shape}")

    X = np.asfortranarray(X)

    r_run_mclust = robjects.r(
        """
        function(values, nrow, ncol, G, modelName, col_names) {
            mat <- matrix(values, nrow = nrow, ncol = ncol, byrow = FALSE)
            colnames(mat) <- col_names
            df <- as.data.frame(mat)
            mclust::Mclust(df, G = G, modelNames = modelName)
        }
        """
    )

    col_names = [f"dim_{i+1}" for i in range(X.shape[1])]

    res = r_run_mclust(
        FloatVector(X.ravel(order="F")),
        IntVector([X.shape[0]])[0],
        IntVector([X.shape[1]])[0],
        IntVector([num_cluster])[0],
        StrVector([modelNames])[0],
        StrVector(col_names),
    )
